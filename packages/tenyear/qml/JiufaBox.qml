...
GraphicsBox {
  ...
  function arrangeCards() {
    ...
    for (i = 0; i < patterns.length; i++) {
      ...
      for (j = 0; j < result[1].length; j++) {
        card = result[1][j];
        if (JSON.parse(Backend.callLuaFunction("CardFitPattern", [card.cid, patterns[i]]))) {
          stay = false;
          break;
        }
      }
      if (stay) {
        tmp_pattern.push(patterns[i])
      }
    }
    ...
    for (i = 0; i < result[0].length; i++) {
      card = result[0][i];
      stay = false;
      for (j = 0; j < tmp_pattern.length; j++) {
        if (JSON.parse(Backend.callLuaFunction("CardFitPattern", [card.cid, tmp_pattern[j]]))) {
          stay = true;
          break;
        }
      }
      card.draggable = stay
      card.selectable = stay
    }
  ...
  function loadData(data) {
    ...
    patterns = d[1]
    areaCapacities = [cards.length, d[3]];
    areaLimits = [0, d[2]];
    prompt = d[4];
    areaNames = [Backend.translate(d[5]), Backend.translate(d[6])];
